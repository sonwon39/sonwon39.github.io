...
{
    p = sin(vec2(
        dot(p.xy , vec2(12.3434, 54.1274)),
        dot(p.xy , vec2(32.4154, 72.9263)))
    );

    return fract(p * 523215.535) * 2. - 1.;
}
...
float remap(float Perlin, float Worley_FBM, float OldMax, float NewMin, float NewMax)
{
    return NewMin + ((Perlin - Worley_FBM) / (OldMax - Worley_FBM)) * (NewMax - NewMin);
}
