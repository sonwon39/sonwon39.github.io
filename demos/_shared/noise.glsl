...
float gnoise(vec3 p, float freq) {
  ...
  vec3 a = mod(floor(p), freq);
  vec3 b = mod(a + v1, freq);
  vec3 c = mod(a + v2, freq);
  vec3 d = mod(a + v3, freq);
  vec3 e = mod(a + v4, freq);
  vec3 f = mod(a + v5, freq);
  vec3 g = mod(a + v6, freq);
  vec3 h = mod(a + v7, freq);
...
float wnoise(vec3 st, float freq) {
  vec3 cell = floor(st);
  vec3 f = fract(st);
  ...
  for (int z = -1; z <= 1; z++) {
    for (int y = -1; y <= 1; y++) {
      for (int x = -1; x <= 1; x++) {
        ...
        vec3 p = hash33(mod(offset + cell, freq)) * 0.5 + 0.5;
...
float worleyFbm(vec3 p, float freq) {
  return (1.0 - wnoise(p * freq, freq)) * 0.625 +
  (1.0 - wnoise(p * freq * 2.0, freq)) * 0.25 +
  (1.0 - wnoise(p * freq * 4.0, freq)) * 0.125;
}
